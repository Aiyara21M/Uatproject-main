...
function TicketNumber() {
  ...
  const sendData = async (e) => {
    ...
    formDataToSend.append("jsonData", JSON.stringify({ content,UUID,depart,id}));
    if (selectedFile) {
      ...
      <div className="flex flex-col-reverse w-full md:flex-row md:h-full  md:w-[95%] md:rounded-lg bg-white overflow-hide">
        <div className="flex-1 h-full md:h-[95%] w-full bg-gray-900 md:rounded-lg md:w-[30%] md:h-[100%] p-4 md:m-4">
          ...
          <div className="flex p-3 md:mt-4">
            <button onClick={sendData} className="w-full p-2 bg-white text-gray-900 font-semibold rounded-lg  shadow-md hover:shadow-lg hover:bg-gray-300  transition-all">
              Send
            </button>
          </div>
        </div>
          ...
          <div className="flex w-full container mx-auto p-1 md:w-[100%] bg-white border-0 border-y-2 border-y-gray-900 rounded-full ">
            <div className="flex flex-col md:flex-row justify-center md:justify-start p-2 items-center w-[100%]">
              ...
              <div className="flex  font-bold text-gray-900 text-md  w-[90%] md:w-[70%]">
                <div className="md:mr-2 w-[35%]"> Ticket - {dataTicket.TicketNumber}</div>
            ...
            {dataTicket.Audilog &&
             ...
             <div>
              <img
                ...
                <div >
              {data.User}
              <div className="text-sm font-none text-gray-400 justify-center items-center h-[100%]">แผนก {data.UserDepartment}</div>
              <div className="text-sm font-none text-gray-400 justify-center items-center h-[100%]">Create Date {data.ModifyDate}</div>
              </div>
                  ...
                  {data.filename && (
                    <div className="mt-3  border-0 border-t-2 pt-2" >
                      Download File
                      <a href={`http://localhost:4211/Uploads/${data.filename}`} >
                          <img src="/img/pngegg(2).png" alt="My Image"  className="h-10 pr-2"   />
                      </a>
                    </div>
                  )}
                </div>
